...
function ChatPane({ thread, seeded, live, permissions, attachments, supportsImages, onAttachments, onPermission, loading, loadError, onRetryLoad, onBack, draft, onDraftChange, sending: externalSending, filesOpen, onToggleFiles }: { thread: ChatThread | null; seeded: ChatMessage[]; live: ChatMessage[]; permissions: PendingPermission[]; attachments: ChatAttachment[]; supportsImages: boolean; onAttachments: (items: ChatAttachment[]) => void; onPermission: (requestId: string, action: "approve" | "deny") => Promise<void>; loading: boolean; loadError: string | null; onRetryLoad: () => void; onBack: () => void; draft: string; onDraftChange: (value: string) => void; sending: boolean; filesOpen: boolean; onToggleFiles: () => void }): JSX.Element {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
     <header className="flex h-14 items-center gap-2 border-b border-border bg-panel/70 px-3 backdrop-blur md:px-5"><Button type="button" size="icon" variant="ghost" className="md:hidden" onClick={onBack} aria-label="Back to threads" title="Back to threads"><ArrowLeft aria-hidden /></Button><div className="min-w-0"><h1 className="truncate text-sm font-semibold tracking-tight">{thread.title}</h1><p className="truncate font-mono text-[0.62rem] text-muted">{thread.agent_id}@{thread.agent_version} · {thread.cwd}</p></div><span className="ml-auto flex items-center gap-1.5 font-mono text-[0.65rem] text-muted"><StatusDot status={thread.status} />{thread.status}</span><Button type="button" size="icon-sm" variant="outline" className="hidden md:inline-flex" onClick={onToggleFiles} aria-label={filesOpen ? "Hide files" : "Show files"} title={filesOpen ? "Hide files" : "Show files"}>{filesOpen ? <ChevronRight aria-hidden /> : <ChevronLeft aria-hidden />}</Button></header>
    <ScrollArea className="min-h-0 flex-1"><div ref={listRef} className="mx-auto max-w-4xl space-y-8 p-4 md:px-10 md:py-8">
      {messages.length === 0 && <p className="py-12 text-center text-sm text-muted">Send a message to begin.</p>}
       {messages.map((message) => <article key={message.id} className={cn(message.role === "user" ? "ml-auto max-w-[85%]" : "mr-auto w-full")}><div className="mb-2 flex items-center gap-2"><p className={cn("font-mono text-[0.62rem] font-semibold tracking-[0.14em] uppercase", message.role === "user" ? "text-primary" : "text-success")}>{message.role === "user" ? "you" : thread.agent_id}</p>{message.role === "user" && <button type="button" className="text-[0.62rem] text-muted hover:text-text" onClick={() => onDraftChange(message.content)}><Pencil aria-hidden className="mr-1 inline size-3" />edit</button>}</div><div className={cn("text-[0.92rem]", message.role === "user" ? "rounded-xl border border-primary/20 bg-accent px-4 py-3" : "border-l-2 border-success/35 pl-4 md:pl-5")}>
    ...
    <div className="border-t border-border bg-panel/85 p-3 backdrop-blur-xl md:px-6 md:py-4"><div className="mx-auto max-w-4xl">
       {(sendError || thread.status === "error") && <div className="mb-2 flex items-center justify-between gap-3 rounded-md border border-error-border bg-error-bg px-3 py-2 text-xs text-error"><span>{sendError ?? "The last turn failed."}</span><Button type="button" size="xs" variant="outline" onClick={() => void send(lastPrompt)} disabled={draftSending || externalSending || !lastPrompt}><RefreshCw aria-hidden />Retry</Button></div>}
        {attachments.length > 0 && <div className="mb-2 flex flex-wrap gap-2">{attachments.map((attachment) => <div key={attachment.id} className="relative"><img src={chatAttachmentUrl(attachment.thread_id, attachment.id)} alt={attachment.filename} className="size-14 rounded border object-cover" /><button type="button" className="absolute -right-1 -top-1 rounded-full bg-panel text-error" onClick={() => onAttachments(attachments.filter((item) => item.id !== attachment.id))} aria-label={`Remove ${attachment.filename}`}><X className="size-3" /></button></div>)}</div>}
          <div className="flex items-end gap-2 rounded-xl border border-input bg-bg p-2 shadow-sm focus-within:border-primary/60 focus-within:ring-2 focus-within:ring-primary/10">{supportsImages && <><input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple className="hidden" onChange={(event) => { if (event.target.files) void upload(event.target.files); event.target.value = ""; }} /><Button type="button" size="icon" variant="ghost" onClick={() => inputRef.current?.click()} disabled={uploading || draftSending || externalSending} aria-label="Attach image" title="Attach image"><ImagePlus aria-hidden /></Button></>}<Textarea className="min-h-12 resize-none border-0 bg-transparent shadow-none focus-visible:ring-0" value={draft} onChange={(event) => onDraftChange(event.target.value)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void upload(event.dataTransfer.files); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder={uploading ? "Uploading image..." : `Message ${thread.agent_id}...`} disabled={draftSending || externalSending} rows={2} /><Button type="button" className="h-9 rounded-lg" onClick={() => void ((draftSending || externalSending) ? cancel() : send())} disabled={!(draftSending || externalSending) && draft.trim().length === 0 && attachments.length === 0} variant={(draftSending || externalSending) ? "destructive" : "default"}>{(draftSending || externalSending) ? <><Square aria-hidden />Stop</> : <><Send aria-hidden />Send</>}</Button></div>
