...
function distributionLabel(kind: DistributionKind): string {
  return kind === "binary" ? "Native download" : kind === "npx" ? "Node package" : "Python package";
}
...
function AgentCard({ agent, installed, operationId, onInstall, onProbe, onAuthenticate, onRemove, onUpdate, onDefault, busy }: { agent: RegistryAgent; installed: InstalledAgent[]; operationId: string | null; onInstall: (distribution: DistributionKind) => void; onProbe: () => void; onAuthenticate: (methodId: string) => void; onRemove: () => void; onUpdate: () => void; onDefault: (installationId: string) => void; busy: boolean }): JSX.Element {
  ...
  const availableKinds = [...new Set(agent.distributions.map((distribution) => distribution.kind))];
  ...
  return <article className="flex min-h-0 flex-col rounded-lg border border-border bg-panel p-4 shadow-sm transition-colors hover:border-primary/50">
    ...
    <p className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-muted">{agent.description}</p>
    <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[0.68rem] font-medium"><span className="rounded-md border border-border px-2 py-1">{agent.license}</span>{availableKinds.map((kind) => <span key={kind} className="rounded-md border border-border px-2 py-1">{distributionLabel(kind)}</span>)}</div>
