...
export function AgentsRoute(): JSX.Element {
  ...
  const authenticateAgent = async (agent: RegistryAgent, methodId: string): Promise<void> => { await authenticate.mutateAsync({ agentId: agent.id, version: agent.version, methodId }); await client.invalidateQueries({ queryKey: queryKeys.installedAgents }); };
...
function AgentCard({ agent, installed, onInstall, onProbe, onAuthenticate, onRemove, busy }: { agent: RegistryAgent; installed?: { id: string; version: string; status: "installing" | "installed" | "failed"; failure: string | null; readiness_status: "unknown" | "probing" | "ready" | "authentication_required" | "failed"; readiness_error: string | null; protocol_version: number | null; capabilities: { prompt: { image: boolean } } | null; auth_methods: { id: string; type: "agent" | "terminal" | "env_var"; name: string; description: string | null }[] }; onInstall: (distribution: "npx" | "uvx" | "binary") => void; onProbe: () => void; onAuthenticate: (methodId: string) => void; onRemove: () => void; busy: boolean }): JSX.Element {
  ...
  return <article className="flex min-h-64 flex-col rounded-xl border border-border bg-panel p-5 shadow-sm transition-colors hover:border-primary/50">
    ...
    <div className="mt-auto flex items-center gap-3 pt-5 text-xs">{agent.repository && <a className="inline-flex items-center gap-1 text-primary hover:underline" href={agent.repository} target="_blank" rel="noreferrer">Source <ExternalLink className="size-3" /></a>}{agent.website && <a className="inline-flex items-center gap-1 text-primary hover:underline" href={agent.website} target="_blank" rel="noreferrer">Website <ExternalLink className="size-3" /></a>}<span className="ml-auto text-muted">{agent.authors[0] ?? "Unknown author"}</span></div>
      <div className="mt-4 flex items-center justify-between gap-2">{installed?.status === "installed" ? <><div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-medium text-success">Installed</span>{installed.readiness_status === "ready" ? <span className="rounded-full bg-success/10 px-2 py-1 font-medium text-success">Ready</span> : installed.readiness_status === "authentication_required" ? <span className="rounded-full bg-warn/10 px-2 py-1 font-medium text-warn">Authentication required</span> : installed.readiness_status === "probing" ? <span className="text-primary">Probing...</span> : installed.readiness_status === "failed" ? <span className="text-danger">Probe failed</span> : null}</div><div className="flex gap-2">{installed.readiness_status === "authentication_required" && installed.auth_methods.filter((method) => method.type === "agent").map((method) => <Button key={method.id} variant="outline" size="sm" onClick={() => onAuthenticate(method.id)} disabled={busy}>Authenticate{installed.auth_methods.length > 1 ? `: ${method.name}` : ""}</Button>)}<Button variant="outline" size="sm" onClick={onProbe} disabled={busy}>{installed.readiness_status === "unknown" ? "Probe readiness" : "Probe again"}</Button><Button variant="outline" size="sm" onClick={onRemove} disabled={busy}>Remove</Button></div></> : installed?.status === "installing" ? <span className="text-xs font-medium text-primary">Installing...</span> : <div className="flex gap-2">{(["npx", "uvx"] as const).filter((kind) => agent.distributions.some((distribution) => distribution.kind === kind)).map((kind) => <Button key={kind} size="sm" onClick={() => onInstall(kind)} disabled={busy}>{installed?.status === "failed" ? `Retry ${kind}` : `Install ${kind}`}</Button>)}</div>}</div>
