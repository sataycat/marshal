...
import { useInstallRegistryAgentMutation, useInstalledAgentsQuery, useRefreshRegistryMutation, useRegistryQuery, useRemoveInstalledAgentMutation } from "../api/queries";
...
function AgentCard({ agent, installed, onInstall, onRemove, busy }: { agent: RegistryAgent; installed?: { status: "installing" | "installed" | "failed"; failure: string | null }; onInstall: () => void; onRemove: () => void; busy: boolean }): JSX.Element {
  return <article className="flex min-h-64 flex-col rounded-xl border border-border bg-panel p-5 shadow-sm transition-colors hover:border-primary/50">
    ...
    <div className="mt-4 flex items-center justify-between gap-2">{installed?.status === "installed" ? <><span className="text-xs font-medium text-success">Installed</span><Button variant="outline" size="sm" onClick={onRemove} disabled={busy}>Remove</Button></> : installed?.status === "installing" ? <span className="text-xs font-medium text-primary">Installing...</span> : <Button size="sm" onClick={onInstall} disabled={busy || !agent.distributions.some((distribution) => distribution.kind === "npx")}>{installed?.status === "failed" ? "Retry installation" : "Install npx"}</Button>}</div>
  </article>;
}
