...
export class ChatTurnRunner {
  ...
  async send(threadId: string, content: string, attachmentIds: string[] = []): Promise<{ userMessage: ChatMessage; assistantMessage: ChatMessage | null }> {
    ...
    if (thread.status === "authentication_required") throw new Error("Authenticate the installed agent, then explicitly resubmit the preserved prompt before sending another message");
    ...
    const userMessage = appendChatMessage(threadId, "user", expandedContent, this.root, attachmentIds);
    ...
    try {
      ...
      if (existing) {
        ...
        try {
          ...
          supervisorSessionId = started.record.id;
           ...
           if (failure.kind === "authentication_required") markChatMessageAuthenticationRequired(userMessage.id, failure, this.root);
           updateChatThread(threadId, { status: failure.kind === "authentication_required" ? "authentication_required" : "error", failure }, this.root);
          this.publishThread(threadId);
          throw err;
        }
      }
      ...
      for (const { attachment, bytes } of attachments) promptParts.push({ type: "image", data: bytes.toString("base64"), mimeType: attachment.mime_type });
       const prompt = this.runPrompt(threadId, supervisorSessionId!, attachments.length === 0 ? expandedContent : promptParts, userMessage.id);
    ...
    return { userMessage, assistantMessage: [...messages].reverse().find((message) => message.role === "assistant" && message.id > userMessage.id) ?? null };
  }

  async resubmit(threadId: string, messageId: number): Promise<{ userMessage: ChatMessage; assistantMessage: ChatMessage | null }> {
    const message = getChatMessage(messageId, this.root);
    if (!message || message.thread_id !== threadId || message.role !== "user" || message.prompt_status !== "authentication_required") throw new Error("Only an authentication-required user message can be resubmitted");
    const thread = getChatThread(threadId, this.root);
    if (this.active.has(threadId) || this.starting.has(threadId)) throw new ChatTurnBusyError(threadId);
    const attachments = message.attachment_ids.map((id) => readChatAttachment(threadId, id, this.root));
    this.getAgent(threadId);
    this.starting.add(threadId);
    try {
      const existing = this.supervisor.sessionForOwner("thread", threadId);
      const started = existing ?? await this.supervisor.start("thread", threadId, thread.cwd, thread.agent_id, thread.agent_version, { agentProvenance: thread.agent_provenance });
      if (attachments.length > 0 && !started.session.supportsImages) throw new Error("This agent does not support ACP image prompts. Remove the images or choose an image-capable agent.");
      const content: string | AgentPromptPart[] = attachments.length === 0 ? message.content : [{ type: "text", text: message.content }, ...attachments.map(({ attachment, bytes }) => ({ type: "image" as const, data: bytes.toString("base64"), mimeType: attachment.mime_type }))];
      const prompt = this.runPrompt(threadId, started.record.id, content, message.id, message.id.toString());
      this.active.set(threadId, { supervisorSessionId: started.record.id, session: started.session, prompt });
      await prompt;
      clearChatMessagePromptFailure(message.id, this.root);
      this.publishMessage(threadId, getChatMessage(message.id, this.root)!);
    } finally { this.starting.delete(threadId); this.active.delete(threadId); }
    const messages = listChatMessages(threadId, this.root);
    return { userMessage: getChatMessage(message.id, this.root)!, assistantMessage: [...messages].reverse().find((item) => item.role === "assistant" && item.id > message.id) ?? null };
  }
  ...
  private async runPrompt(threadId: string, supervisorSessionId: string, content: string | AgentPromptPart[], messageId: number, resubmissionOf?: string): Promise<void> {
    ...
    try {
      ...
      this.publishThread(threadId);
       ...
       const failure = structuredAcpError(err);
       if (failure.kind === "authentication_required") { const message = markChatMessageAuthenticationRequired(messageId, failure, this.root); this.publishMessage(threadId, message); }
       updateChatThread(threadId, { status: failure.kind === "authentication_required" ? "authentication_required" : "error", failure }, this.root);
      this.publishThread(threadId);
      throw err;
    }
  ...
  private getAgent(threadId: string): Agent {
    ...
    const thread = getChatThread(threadId, this.root);
    const installed = getInstalledAgent(thread.agent_id, thread.agent_version, this.machineDir, thread.agent_provenance.installation_id ?? undefined);
