...
const testAgent: Agent = {
  async spawn(cwd, agentId) { return { cwd, agentId, name: "test", recordId: "test-session" }; },
...
describe("chat thread API", () => {
  it("creates, opens, updates, and appends to a thread without starting an agent", async () => {
    ...
    const created = await req(app, "POST", "/api/threads", { agent_id: "agent-a", agent_version: "1.0.0", title: "Debug login" });
    expect(created.status).toBe(201);
    const id = created.body.thread.id;
    expect(created.body.thread).toMatchObject({ agent_id: "agent-a", agent_version: "1.0.0", title: "Debug login", status: "draft" });
  ...
  it("rejects unknown thread fields and missing agent IDs", async () => {
    ...
    expect((await req(app, "POST", "/api/threads", { agent_id: "a", agent_version: "1.0.0", nope: true })).status).toBe(400);
  });
  ...
  it("pins a ready installed agent version and rejects unready production selections", async () => {
    ...
    const input = { id: "agent-a", version: "1.2.3", source: "registry" as const, license: "MIT", distribution: "npx" as const, package_specifier: "agent-a@1.2.3", launch: { command: "npx" as const, args: ["--yes", "agent-a@1.2.3"] }, registry_snapshot_fetched_at: "fixture", integrity_status: "not_applicable" as const, status: "installing" as const, readiness_status: "unknown" as const, readiness_error: null, protocol_version: null, capabilities: null, auth_methods: [], raw_initialize: null, probed_at: null };
    ...
    const unready = await req(app, "POST", "/api/threads", { agent_id: input.id, agent_version: input.version });
    expect(unready).toMatchObject({ status: 409, body: { code: "agent_not_ready" } });
    setAgentReadiness(input.id, input.version, { readiness_status: "ready", readiness_error: null, protocol_version: 1, capabilities: { prompt: { text: true, image: false, audio: false, embedded_context: false }, session: { close: true, list: false, load: false, fork: false, resume: false }, load_session: false, auth: { logout: false } }, auth_methods: [], raw_initialize: {}, probed_at: new Date().toISOString() }, machineDir);
    const created = await req(app, "POST", "/api/threads", { agent_id: input.id, agent_version: input.version });
  ...
  it("supports lifecycle actions and deletion", async () => {
    ...
    initGitRepo(root);
    const app = buildApp("0.0.1", { root, chatAgent: testAgent });
    const created = await req(app, "POST", "/api/threads", { agent_id: "agent-a", agent_version: "1.0.0" });
    const id = created.body.thread.id;
