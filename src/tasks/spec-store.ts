...
export class SpecMessageNotFoundError extends Error {
  constructor(id: number) {
    super(`Spec message not found: ${id}`);
    this.name = "SpecMessageNotFoundError";
  }
}
...
function rowToMessage(row: SpecMessageRow): SpecMessage {
  return { ...row, repository_id: row.repository_id, role: asRole(row.role), prompt_status: row.prompt_status === "authentication_required" ? "authentication_required" : null, failure: row.failure ? JSON.parse(row.failure) as StructuredAcpError : null };
}
...
export function listSpecMessages(first: string, second?: string, third?: string): SpecMessage[] {
  ...
  const rows = db
    .prepare(
       scoped ? "SELECT id, repository_id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE repository_id = ? AND task_id = ? ORDER BY id ASC" : "SELECT id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE task_id = ? ORDER BY id ASC",
    )
...
export function appendSpecMessage(first: string, second: string, third: string, fourth?: string, fifth?: string): SpecMessage {
  ...
  const row = db
    .prepare(scoped ? "SELECT id, repository_id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE repository_id = ? AND id = ?" : "SELECT id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE id = ?")
...
export function getSpecMessage(first: string | number, second?: number | string, third?: string): SpecMessage | undefined { const scoped = typeof first === "string"; const id = scoped ? second as number : first; const db = scoped ? openRepositoryDb(first, third) : openDb(second as string | undefined); const row = db.prepare(scoped ? "SELECT id, repository_id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE repository_id = ? AND id = ?" : "SELECT id, task_id, role, content, prompt_status, failure, created_at FROM spec_messages WHERE id = ?").get(...(scoped ? [first, id] : [id])) as SpecMessageRow | undefined; return row ? rowToMessage(row) : undefined; }
