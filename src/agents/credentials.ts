...
export function bindAgentCredential(
  ...
  const existing = db
    .prepare(
      "SELECT credential_ref FROM agent_credential_bindings WHERE installation_id = ? AND variable_name = ?",
    )
    .get(installationId, variableName) as { credential_ref: string } | undefined;
  ...
  db.prepare(`INSERT INTO agent_credential_bindings (installation_id, variable_name, credential_ref, secret, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(installation_id, variable_name) DO UPDATE SET credential_ref = excluded.credential_ref, secret = excluded.secret, updated_at = excluded.updated_at`).run(
    installationId,
    variableName,
    reference,
    secret ? 1 : 0,
  ...
  return getAgentCredentialBindings(installationId, machineDir).find(
    (binding) => binding.variable_name === variableName,
  )!;
}

export function getAgentCredentialBindings(
  installationId: string,
  machineDir?: string,
): CredentialBinding[] {
  return (
    table(machineDir)
      .prepare(
        "SELECT * FROM agent_credential_bindings WHERE installation_id = ? ORDER BY variable_name",
      )
...
export function resolveAgentCredentialValues(
  ...
  return Object.fromEntries(
    getAgentCredentialBindings(installationId, machineDir).map((binding) => [
      binding.variable_name,
      store.get(binding.credential_ref),
    ]),
  );
}
...
export function deleteAgentCredentials(
  ...
  table(machineDir)
    .prepare("DELETE FROM agent_credential_bindings WHERE installation_id = ?")
    .run(installationId);
}
