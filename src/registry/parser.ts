...
function archiveFormat(
  ...
  value: unknown,
  field: string,
): "tar.gz" | "tgz" | "tar.bz2" | "zip" | undefined {
  const format =
    typeof value === "string" ? value : url.match(/\.(tar\.bz2|tar\.gz|tgz|zip)(?:$|\?)/i)?.[1];
  if (!format) return undefined;
  if (!ARCHIVE_FORMATS.has(format.toLowerCase()))
    throw new RegistryValidationError(`${field}.format is unsupported`);
...
function safePath(value: unknown, field: string): string {
  ...
  if (
    ...
    throw new RegistryValidationError(`${field} must be a safe relative path`);
  return path;
}
...
function archive(
  ...
  "archive_url" | "archive_format" | "checksum" | "executable" | "args" | "env" | "platform"
> {
  ...
  try {
    ...
    throw new RegistryValidationError(`${field}.url must be a valid URL`);
  }
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:")
    throw new RegistryValidationError(`${field}.url must use http or https`);
  ...
  const checksum =
    checksumValue === undefined || checksumValue === null
      ? undefined
      : requiredString(checksumValue, `${field}.checksum`)
          .replace(/^sha256:/i, "")
          .toLowerCase();
  if (checksum !== undefined && !/^[a-f0-9]{64}$/.test(checksum))
    throw new RegistryValidationError(`${field}.checksum must be a SHA-256 digest`);
  ...
  if (
    ...
    throw new RegistryValidationError(`${field}.env must contain safe string environment metadata`);
  return {
    platform: optionalString(value.platform, `${field}.platform`),
    archive_url: url,
    ...(format ? { archive_format: format } : {}),
    checksum,
    executable: safePath(
      value.executable ?? value.executable_path ?? value.cmd,
      `${field}.executable`,
    ),
    args: value.args === undefined ? [] : stringArray(value.args, `${field}.args`),
...
function distributions(value: unknown, index: number): RegistryDistribution[] {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new RegistryValidationError(`agents[${index}].distribution must be an object`);
  ...
  if (distribution.npx !== undefined) {
    ...
    if (npx === null || typeof npx !== "object" || Array.isArray(npx))
      throw new RegistryValidationError(`agents[${index}].distribution.npx must be an object`);
    result.push({
      kind: "npx",
      package: requiredString(
        (npx as Record<string, unknown>).package,
        `agents[${index}].distribution.npx.package`,
      ),
      args:
        (npx as Record<string, unknown>).args === undefined
          ? []
          : stringArray(
              (npx as Record<string, unknown>).args,
              `agents[${index}].distribution.npx.args`,
            ),
    });
  }
  if (distribution.uvx !== undefined) {
    const uvx = distribution.uvx;
    if (uvx === null || typeof uvx !== "object" || Array.isArray(uvx))
      throw new RegistryValidationError(`agents[${index}].distribution.uvx must be an object`);
    result.push({
      kind: "uvx",
      package: requiredString(
        (uvx as Record<string, unknown>).package,
        `agents[${index}].distribution.uvx.package`,
      ),
      args:
        (uvx as Record<string, unknown>).args === undefined
          ? []
          : stringArray(
              (uvx as Record<string, unknown>).args,
              `agents[${index}].distribution.uvx.args`,
            ),
    });
  }
  if (distribution.binary !== undefined) {
    if (
      ...
      Array.isArray(distribution.binary)
    )
      throw new RegistryValidationError(`agents[${index}].distribution.binary must be an object`);
    ...
    if (platforms.length === 0)
      throw new RegistryValidationError(
        `agents[${index}].distribution.binary must list a platform`,
      );
    for (const platform of platforms) {
      if (!PLATFORMS.has(platform))
        throw new RegistryValidationError(
          `agents[${index}].distribution.binary.${platform} is an unsupported platform`,
        );
      const value = binary[platform];
      if (value === null || typeof value !== "object" || Array.isArray(value))
        throw new RegistryValidationError(
          `agents[${index}].distribution.binary.${platform} must be an object`,
        );
      result.push({
        kind: "binary",
        platforms: [platform],
        ...archive(
          ...
          `agents[${index}].distribution.binary.${platform}`,
        ),
      });
    }
  }
  if (result.length === 0)
    throw new RegistryValidationError(
      `agents[${index}].distribution must provide a supported distribution`,
    );
  return result;
}

export function parseRegistryDocument(value: unknown): {
  version: string;
  agents: RegistryAgent[];
} {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new RegistryValidationError("registry document must be an object");
  const document = value as Record<string, unknown>;
  const version = requiredString(document.version, "version");
  if (!/^1\./.test(version))
    throw new RegistryValidationError(`unsupported registry version: ${version}`);
  ...
  const agents = document.agents.map((value, index) => {
    if (value === null || typeof value !== "object" || Array.isArray(value))
      throw new RegistryValidationError(`agents[${index}] must be an object`);
    ...
    if (
      ...
      throw new RegistryValidationError(`agents[${index}].authors must be an array of strings`);
    return {
