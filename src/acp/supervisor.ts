...
export interface SupervisorOptions {
  /** Required immutable persistence owner. */
  repositoryId: string;
  /** Execution checkout metadata, never a persistence selector. */
  root?: string;
  machineDir?: string;
  bus?: EventBus;
  agent?: Agent;
  ...
  onSessionConfiguration?: (
    ...
    configuration: { configOptions: AgentSessionConfigOption[]; modes: AgentSessionModeState | null },
  ) => void;
}
...
export class AcpSessionSupervisor {
  ...
  constructor(options: SupervisorOptions) {
    ...
    interruptActiveSessions(options.repositoryId, options.machineDir);
  }

  reconcile(): void {
    for (const record of listSessions(this.options.repositoryId, this.options.machineDir)) {
      if (
  ...
  getSession(id: string): AcpSessionRecord | undefined {
    return getSession(this.options.repositoryId, id, this.options.machineDir);
  }
  ...
  sessionForOwner(ownerType: string, ownerId: string): { record: AcpSessionRecord; session: AgentSession } | undefined {
    for (const runtime of this.runtimes.values()) {
      if (runtime.record.owner_type === ownerType && runtime.record.owner_id === ownerId) {
        return {
          record: getSession(this.options.repositoryId, runtime.record.id, this.options.machineDir)!,
  ...
  events(id: string) {
    return listSessionEvents(this.options.repositoryId, id, this.options.machineDir);
  }
  ...
  async start(
    ...
    agentId: string,
    agentVersion: string,
    config: { model?: string | null; mode?: string | null; sessionName?: string; agentProvenance?: HistoricalAgentProvenance } = {},
  ): Promise<{ record: AcpSessionRecord; session: AgentSession }> {
    const installed = this.options.agent
      ? null
      : getInstalledAgent(agentId, agentVersion, this.options.machineDir, config.agentProvenance?.installation_id ?? undefined);
    ...
    const record = createSession(this.options.repositoryId, {
      ownerType,
      ownerId,
      agentId,
      agentVersion,
    ...
    try {
      const session = await withTimeout(
        agent.spawn(cwd, agentId, {
          permissionMode: this.options.permissionMode ?? (this.options.workflow ? "deny-all" : "interactive"),
          model: config.model ?? undefined,
          sessionName: config.sessionName ?? `marshal-${ownerId}`,
          onSessionConfiguration: (configuration) => this.recordConfiguration(record.id, configuration),
        }),
        30_000,
        "ACP session startup timed out",
      );
      const updated = updateSession(this.options.repositoryId, record.id, {
        acp_session_id: session.recordId ?? null,
        status: "idle",
        started_at: new Date().toISOString(),
        capabilities: { image: session.supportsImages === true, configOptions: session.configOptions ?? [], modes: session.modes ?? null },
        recovery_metadata: { resumable: false, session_name: session.name },
      }, this.options.machineDir);
      this.runtimes.set(record.id, { record: updated, session, agent });
      this.recordConfiguration(record.id, { configOptions: session.configOptions ?? [], modes: session.modes ?? null });
      return { record: updated, session };
    } catch (err) {
      const failure = structuredAcpError(err);
      updateSession(this.options.repositoryId, record.id, {
  ...
  async setConfigOption(id: string, configId: string, value: string | boolean): Promise<AgentSessionConfigOption[]> {
    const runtime = this.runtimes.get(id);
    if (!runtime) throw new Error(`ACP session is not active: ${id}`);
    if (!runtime.agent.setConfigOption) throw new Error("This agent does not support ACP session configuration");
  ...
  permissions(threadId: string): PermissionRequestRecord[] {
    return listPermissionRequests(this.options.repositoryId, threadId, this.options.machineDir);
  }

  decidePermission(threadId: string, requestId: string, action: "approve" | "deny"): PermissionRequestRecord {
    const request = getPermissionRequestForThread(this.options.repositoryId, threadId, requestId, this.options.machineDir);
    ...
    return resolvePermissionRequest(this.options.repositoryId, request.id, action === "approve" ? "approved" : "denied", option.optionId, action, null, this.options.machineDir);
  }
  ...
  async prompt(
    ...
    const runtime = this.runtimes.get(id);
    if (!runtime) throw new Error(`ACP session is not active: ${id}`);
    const prompt = createPrompt(this.options.repositoryId, id, content, this.options.machineDir, durable);
    runtime.prompt = prompt;
    updateSession(this.options.repositoryId, id, { status: "running", failure: null, diagnostic: null }, this.options.machineDir);
    try {
      const onPermission = async (request: AgentPermissionRequest): Promise<string | undefined> => {
        ...
        const automatic = policy === "reject_all" || (this.options.workflow && policy === "allow_reads_ask_writes")
        ...
        const record = createPermissionRequest(this.options.repositoryId, id, threadId, request, this.options.machineDir);
        this.options.onPermission?.(record);
        this.options.bus?.publish("thread.event", { repositoryId: this.options.repositoryId, threadId, event: { type: "permission-request", request: record } });
        return await new Promise<string | undefined>((resolve) => {
          const timer = setInterval(() => {
            const current = getPermissionRequestByRequestId(this.options.repositoryId, id, request.requestId, this.options.machineDir);
      ...
      for await (const event of runtime.agent.prompt(runtime.session, content, {
        ...
        appendEvent(this.options.repositoryId, id, prompt.id, event.type, event, event, this.options.machineDir);
        this.options.onEvent?.(getSession(this.options.repositoryId, id, this.options.machineDir)!, event);
        onEvent?.(event);
        if (event.type === "error") throw event.failure ? new StructuredAcpFailureError(event.failure) : new Error(event.message);
      }
      updatePrompt(this.options.repositoryId, prompt.id, { status: "completed", ended_at: new Date().toISOString() }, this.options.machineDir);
      updateSession(this.options.repositoryId, id, { status: "idle" }, this.options.machineDir);
    } catch (err) {
      const failure = structuredAcpError(err);
      const cancelled = failure.kind === "cancelled";
      const authRequired = failure.kind === "authentication_required";
      reconcilePermissionRequests(this.options.repositoryId, id, cancelled ? "cancelled" : "interrupted", failure.message, this.options.machineDir);
      updatePrompt(this.options.repositoryId, prompt.id, { status: authRequired ? "authentication_required" : cancelled ? "cancelled" : "failed", diagnostic: failure.message, failure, ended_at: new Date().toISOString() }, this.options.machineDir);
      updateSession(this.options.repositoryId, id, { status: authRequired ? "authentication_required" : cancelled ? "cancelled" : "failed", diagnostic: failure.message, failure, ended_at: new Date().toISOString() }, this.options.machineDir);
  ...
  async cancel(id: string): Promise<void> {
    const runtime = this.runtimes.get(id);
    if (!runtime) return;
    if (runtime.prompt) updatePrompt(this.options.repositoryId, runtime.prompt.id, { cancellation_requested_at: new Date().toISOString() }, this.options.machineDir);
    reconcilePermissionRequests(this.options.repositoryId, id, "cancelled", "ACP session cancelled", this.options.machineDir);
    updateSession(this.options.repositoryId, id, { status: "cancelling" }, this.options.machineDir);
  ...
  async close(id: string): Promise<void> {
    const runtime = this.runtimes.get(id);
    if (!runtime) return;
    reconcilePermissionRequests(this.options.repositoryId, id, "cancelled", "ACP session closed", this.options.machineDir);
    await withTimeout(runtime.agent.close(runtime.session), 10_000, "ACP shutdown timed out");
    updateSession(this.options.repositoryId, id, { status: "closed", ended_at: new Date().toISOString() }, this.options.machineDir);
  ...
  private recordConfiguration(id: string, configuration: { configOptions: AgentSessionConfigOption[]; modes: AgentSessionModeState | null }): void {
    const current = getSession(this.options.repositoryId, id, this.options.machineDir);
    if (!current) return;
    const capabilities = current.capabilities && typeof current.capabilities === "object" ? current.capabilities as Record<string, unknown> : {};
    const updated = updateSession(this.options.repositoryId, id, { capabilities: { ...capabilities, configOptions: configuration.configOptions, modes: configuration.modes } }, this.options.machineDir);
    const runtime = this.runtimes.get(id);
