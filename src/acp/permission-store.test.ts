...
describe("durable permission requests", () => {
  it("persists raw ACP choices and reconciles unresolved requests", () => {
    ...
    const thread = createChatThread(repository.id, { agentId: "fake", agentVersion: "1" }, machine);
    const session = createSession(repository.id, { ownerType: "thread", ownerId: thread.id, agentId: "fake", agentVersion: "1" }, machine);
    const request = createPermissionRequest(repository.id, session.id, thread.id, { requestId: "req", sessionId: "acp", tool: "write", kind: "file", rawInput: { path: "x" }, options: [{ optionId: "deny-id", name: "No", kind: "reject_once" }, { optionId: "allow-id", name: "Yes", kind: "allow_once" }] }, machine);
    expect(getPermissionRequestForThread(repository.id, thread.id, "req", machine)?.options[1].optionId).toBe("allow-id");
    ...
    const second = createPermissionRequest(repository.id, session.id, thread.id, { requestId: "req-2", sessionId: "acp", tool: "run", options: [] }, machine);
    reconcilePermissionRequests(repository.id, session.id, "interrupted", "daemon restart", machine);
    expect(listPermissionRequests(repository.id, thread.id, machine).find((item) => item.id === second.id)?.status).toBe("interrupted");
  });
});
